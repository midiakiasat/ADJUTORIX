...
function makeBridge(): MockBridge {
  ...
  return {
    ...
    workspace: {
      load: vi.fn(async ({ workspaceId }: { workspaceId: string }) => {
...
describe("smoke/restore_workspace_session", () => {
  ...
  it("restores persisted workspace session, reattaches governed surfaces, and reaches visible ready state", async () => {
    ...
    await waitFor(() => {
      expect(bridge.session.restore).toHaveBeenCalledTimes(1);
      expect(bridge.workspace.load).toHaveBeenCalledWith({ workspaceId: "ws-restore-1" });
    });
    ...
    expect(bridge.diagnostics.load).toHaveBeenCalledWith({ workspaceId: "ws-restore-1" });
  ...
  it("does nothing destructive when no restorable session exists and remains in ordinary cold-boot state", async () => {
    ...
    expect(bridge.workspace.load).not.toHaveBeenCalledWith({ workspaceId: "ws-restore-1" });
  });
