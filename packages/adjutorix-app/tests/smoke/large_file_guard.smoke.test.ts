  ...
  return {
    ...
    diagnostics: {
      load: vi.fn(async ({ workspaceId, selectedPath }: { workspaceId: string; selectedPath?: string | null }) => ({
        workspaceId,
        selectedPath: selectedPath ?? LARGE_FILE_PATH,
        diagnostics: [],
        summary: {
          total: 0,
          fatalCount: 0,
...
describe("smoke/large_file_guard", () => {
  ...
  it("guards an oversized file, avoids normal edit mode, and surfaces a safe preview/degraded editor state", async () => {
    ...
    const fileNode =
      screen.queryByText(new RegExp(LARGE_FILE_NAME, "i")) ??
      screen.queryByRole("treeitem", { name: new RegExp(LARGE_FILE_NAME, "i") }) ??
      screen.getByText(/logs/i);
    ...
    await waitFor(() => {
      expect(bridge.files.stat).toHaveBeenCalledWith({ path: LARGE_FILE_PATH });
      expect(bridge.files.read).toHaveBeenCalledWith({ path: LARGE_FILE_PATH });
    });
